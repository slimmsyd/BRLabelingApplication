...
- Key components:
  ...
  - `src/components/workspace/VideoPlayer.tsx` - Video annotation interface (55KB).
    Multi-camera switching is cancellable; see Change Log 2026-08-05.
...
### 2026-08-05 12:56 EDT — Multi-camera switching made cancellable

**Files:** `src/components/workspace/VideoPlayer.tsx`, `src/lib/video-trace.ts` (new),
`scripts/repro-camera-switch-freeze.ts` (new), `scripts/verify-video-trace.ts` (new)

**Reported symptom:** switching between camera angles sometimes froze the picture.
Certain controls stopped responding, and a camera the labeler could not see was
audibly playing.

#### Previous design (as built in VIDEO_SWITCHING_FIX.md)

All cameras render as persistent sibling `<video>` elements; only the active one is
visible via `display`. Switching was a `useEffect` on `activeCam` that ran a
four-step asynchronous sequence:

1. read the outgoing camera's `currentTime`, pause it
2. assign that time to the incoming camera's `currentTime`
3. register a **one-shot `seeked` listener** plus a 500ms fallback timer
   (cold cameras first waited on a `canplay` listener, then did the same)
4. on fire: reassign the shared `videoRef.current`, then call `play()`

`videoRef` is a single mutable ref owned by `workspace/page.tsx` and shared with the
event-logging sidebar, so whichever handler fired last determined what every control
operated on.

**The flaw:** that effect returned **no cleanup function.** React re-ran it on each
`activeCam` change, but nothing detached the previous switch's listeners or cleared
its timers. A superseded switch stayed armed and ran to completion later, against a
camera the user had already left. Consequences, all reproduced:

- `play()` called on a `display:none` element (hidden camera playing)
- `videoRef.current` retargeted to that hidden element, so play/pause, frame-step,
  skip and scrub all drove the wrong video while the visible one sat still
- `isSwitchingCamera` was set `true` on the cold-camera path and cleared only by the
  switch that raised it, so abandoning a cold camera left the `z-50 inset-0`
  "Loading camera..." overlay permanently swallowing clicks
- `togglePlay` branched on the React `isPlaying` state rather than the element's real
  `paused` flag, so once state desynced the button did the opposite of its icon

Network speed was never the cause. It only set how wide the race window was, which is
why the bug looked intermittent and resisted manual reproduction. `preload="metadata"`
widened it further: inactive cameras report "ready" on metadata alone while still
being `readyState < 1` for playback, pushing more switches onto the slow path.

#### New design

The switch effect is now **cancellable**. Each run takes a monotonic `mySwitchId` from
`switchIdRef` and registers every listener and timer it arms into a local `teardown`
array. The effect returns a cleanup that sets `cancelled`, runs all teardowns, and
clears `isSwitchingCamera`.

Every asynchronous handler now opens with an `isCurrent()` guard
(`!cancelled && mySwitchId === switchIdRef.current`) and returns early if its switch
has been superseded — before touching `videoRef` or calling `play()`. Abandoned
switches record a `switch.abandoned` trace event instead of acting.

`togglePlay` now resolves the on-screen element through `getActiveVideoElement()` and
branches on `video.paused`, so it cannot be desynced by React state.

**Invariant this establishes:** at most one camera switch is ever live, and only the
switch matching the camera currently on screen may mutate `videoRef` or start playback.

#### Supporting tooling

- `src/lib/video-trace.ts` — a ring-buffer flight recorder, off by default, enabled per
  session with `?trace=video` on the workspace URL. Auto-flags hidden-camera-playing,
  videoRef divergence, frozen visible camera, stuck overlay, and wrong-target control
  presses. A labeler runs `__videoTrace.copy()` after a freeze to hand over a timeline.
  The old per-switch emoji `console.log` blocks were retired in its favour; the useful
  media events (`stalled`, `waiting`, `error`) now record into the same timeline.
- `scripts/repro-camera-switch-freeze.ts` — renders the real `VideoPlayer` under jsdom
  through React's real effect lifecycle, replacing only the media layer with a fake
  `<video>` whose seek completes on command. Turns an intermittent network-dependent
  bug into a deterministic offline test. Added `jsdom` as a devDependency for this.
- `scripts/verify-video-trace.ts` — 24 checks on the recorder itself.

---

*Architecture analysis: 2026-04-30*
*Last updated: 2026-08-05 12:56 EDT*
