...
const FeedbackWidget = () => {
    ...
    return (
        <div className={`bg-[#161616] rounded-2xl overflow-hidden border transition-colors ${open ? 'border-accent-primary/30' : 'border-border'}`}>
            ...
            <button
                onClick={() => setOpen(o => !o)}
                className="w-full flex items-center gap-3 p-4 text-left hover:bg-surface-hover/30 transition-colors"
            >
                ...
                <div className="px-4 pb-4">
                    ...
                    {sent ? (
                        <div className="flex flex-col items-center gap-3 py-5">
                            ...
                            <button
                                onClick={reset}
                                className="px-3.5 py-1.5 text-xs font-semibold rounded-lg bg-white/5 text-foreground-secondary border border-border hover:bg-surface-hover transition-colors"
                            >
                                Send another
                            </button>
                        </div>
                        ...
                        <div className="flex flex-col gap-3.5">
                                ...
                                {CATS.map(c => {
                                    ...
                                    return (
                                        <button
                                            key={c.k}
                                            onClick={() => setCat(c.k)}
                                            className={`px-2.5 py-1 text-[11.5px] font-semibold rounded-full border transition-all ${on ? c.active : 'bg-white/[0.03] text-foreground-secondary border-border hover:bg-surface-hover'}`}
                                        >
                                            {c.label}
                                        </button>
                                    );
                                    ...
                                    {[1, 2, 3, 4, 5].map(n => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`flex-1 h-[30px] rounded-lg border flex items-center justify-center transition-all ${fill ? 'bg-accent-primary/10 border-accent-primary/50' : 'bg-white/[0.03] border-border'}`}
                                            >
                            ...
                            <div className="flex items-center gap-2.5">
                                ...
                                <button
                                    ...
                                    className={`ml-auto inline-flex items-center gap-1.5 px-4 py-2 text-[12.5px] font-bold rounded-[9px] transition-all ${canSend && !submitting ? 'bg-accent-primary text-white hover:bg-accent-primary/90' : 'bg-[#1f1f23] text-zinc-600 cursor-not-allowed'}`}
                                >
