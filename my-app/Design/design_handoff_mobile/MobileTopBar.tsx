...
const MobileTopBar = ({ onMenu }: { onMenu: () => void }) => {
    return (
        <div className="lg:hidden sticky top-0 z-30 -mx-4 -mt-4 mb-2 px-4 h-14 flex items-center gap-3 bg-background/90 backdrop-blur-md border-b border-border">
            <button
                onClick={onMenu}
                aria-label="Open menu"
                className="w-9 h-9 -ml-1 flex items-center justify-center rounded-lg text-foreground-secondary hover:text-foreground hover:bg-white/5 transition-colors"
            >
                <Menu size={22} />
            </button>
