/**
 * Export Events Log Report (Feb 2-6, 2026)
 *
 * Generates 3 CSV files:
 * 1. events-created-feb-2-6 - All events CREATED in date range
 * 2. events-modified-feb-2-6 - All events MODIFIED in date range (QC changes)
 * 3. qc-changes-feb-2-6 - All Olly's and Dan's changes with original labeler info
 */
...
// Date range: Feb 2-6, 2026
const START_DATE = new Date('2026-02-02T00:00:00.000Z');
const END_DATE = new Date('2026-02-06T23:59:59.999Z');

// QC Reviewers
const QC_EMAILS = ['dan@boxraw.com', 'olly@boxraw.com'];
...
async function main() {
  console.log('================================================');
  console.log('   EVENT LOG REPORT: Feb 2-6, 2026');
  console.log('================================================');
  ...
  if (createdRows.length > 0) {
    ...
    const createdFilepath = saveCSV(createdCSV, `events-created-feb-2-6-${timestamp}.csv`);
  ...
  // ============================================
  console.log('\n\n=== 2. EVENTS MODIFIED (Feb 2-6) - QC Changes ===');
  ...
  if (modifiedRows.length > 0) {
    ...
    const modifiedFilepath = saveCSV(modifiedCSV, `events-modified-feb-2-6-${timestamp}.csv`);
  ...
  // ============================================
  // 3. Olly's and Dan's Changes (from both created & modified)
  // ============================================
  console.log('\n\n=== 3. QC CHANGES - OLLY & DAN (Feb 2-6) ===');

  const allRows = [...createdRows, ...modifiedRows];
  const qcRows = allRows.filter(r => r.labeledByEmail && QC_EMAILS.includes(r.labeledByEmail));

  console.log(`Found ${qcRows.length} events where Olly or Dan made changes`);

  if (qcRows.length > 0) {
    const qcCSV = generateCSV(qcRows);
    const qcFilepath = saveCSV(qcCSV, `qc-changes-feb-2-6-${timestamp}.csv`);
    console.log(`Exported to: ${qcFilepath}`);
    ...
    console.log('\nBy Original Labeler (whose work was reviewed):');
    ...
    const actualQCCount = qcRows.filter(r => r.isQC).length;
    console.log(`\nActual QC changes (reviewing others): ${actualQCCount}`);
    console.log(`Own work: ${qcRows.length - actualQCCount}`);
  } else {
    console.log('Olly and Dan did not make any changes in this date range');
  }
  ...
  console.log('================================================');
  console.log(`Events created (Feb 2-6): ${createdRows.length}`);
  console.log(`Events modified (Feb 2-6): ${modifiedRows.length}`);
  console.log(`QC total changes (Olly & Dan): ${qcRows.length}`);

  // Breakdown by QC reviewer
  const danCount = qcRows.filter(r => r.labeledByEmail === 'dan@boxraw.com').length;
  const ollyCount = qcRows.filter(r => r.labeledByEmail === 'olly@boxraw.com').length;
  console.log(`  - Dan: ${danCount}`);
  console.log(`  - Olly: ${ollyCount}`);
